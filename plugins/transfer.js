    ...
    if (args.length < 3) {
        ...
        switch (type) {
            case 'money':
                ...
                break
            case 'sword':
                if (global.DATABASE._data.users[m.sender].sword >= count * 1) {
                    try {
                        global.DATABASE._data.users[m.sender].sword -= count * 1
                        global.DATABASE._data.users[who].sword += count * 1
                        conn.reply(m.chat, `Berhasil mentransfer ${count} Sword`.trim(), m)
                    } catch (e) {
                        global.DATABASE._data.users[m.sender].sword += count * 1
                        m.reply('Gagal Menstransfer')
                        console.log(e)
                ...
                break
                case 'armor':
                if (global.DATABASE._data.users[m.sender].armor>= count * 1) {
                    try {
                        global.DATABASE._data.users[m.sender].armor-= count * 1
                        global.DATABASE._data.users[who].armor+= count * 1
                        conn.reply(m.chat, `Berhasil mentransfer ${count} Armor`.trim(), m)
                    } catch (e) {
                        global.DATABASE._data.users[m.sender].armor+= count * 1
                        m.reply('Gagal Menstransfer')
                        console.log(e)
