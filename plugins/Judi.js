...
let handler = async (m, { conn, args, usedPrefix, DevMode }) => {
    ...
    try {
        ...
        if (global.DATABASE._data.users[m.sender].money >= count * 1) {
            ...
            if (Aku > Kamu) {
                conn.sendButton(m.chat, `Aku Roll :${Aku}\nKamu Roll : ${Kamu}\n\nKamu *Kalah*, Kamu Kehilangan ${count} Money`.trim(), 'mau judi lagi? By Hanzz Botz', 'Dompet', '.dompet', m)
            } else if (Aku < Kamu) {
                global.DATABASE._data.users[m.sender].money += count * 2
                conn.sendButton(m.chat, `Aku Roll :${Aku}\nKamu Roll : ${Kamu}\n\nKamu *Menang*, Kamu Mendapatkan ${count * 2} Money`.trim(), 'mau judi lagi? By Hanzz Botz', 'Dompet', '.dompet', m)
            } else {
                global.DATABASE._data.users[m.sender].money += count * 1
                conn.sendButton(m.chat, `Aku Roll :${Aku}\nKamu Roll : ${Kamu}\n\nKamu *Seri*, Kamu Mendapatkan ${count * 1} Money`.trim(), 'mau judi lagi? By Hanzz Botz', 'Dompet', '.dompet', m)
            }
        } else conn.sendButton(m.chat, `Uang Kamu Tidak Cukup Untuk Melakukan Judi Sebesar ${count} Money`.trim(), 'Silahkan Claim Untuk Mendapatkan Beberapa Money', 'Claim', '.claim', m)
    } catch (e) {
        console.log(e)
        m.reply('Error!!')
        if (DevMode) {
